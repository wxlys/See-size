...
旧机 SeeSize Hub、Agent 和备份定时器已停用；删除四个专属 systemd 单元及确认归属的 `/opt/seesize-dev` 目录（包含旧库、程序、凭据、测试文件和备份）。未清理全局 journald 或系统用户，避免影响主机其他内容。此后不再使用 SSH 别名 18。

新 Hub 在事务中删除 `aliyun-test-18`、`wsrser-validation-20260913` 两条旧设备记录：50150 条指标、211 个快照、2 条设备凭据记录及2条服务器记录；无对应告警或注册码。保留 `wsrser-main`。SQLite quick_check 为 ok，Hub/Agent active、healthz 正常。

新服务器的既有备份、迁移归档与本地受限中转归档未删除，其中仍可能包含旧历史；上述删除针对在线数据库，不代表所有副本不可恢复。恢复这些旧备份时须重新核对设备身份和撤销状态。以下正文为迁移当时的记录，旧机“暂未停止”状态已被本节更新取代。
