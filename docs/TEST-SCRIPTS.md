...
2026-09-22 更新：旧机已退役，以下 SSH 18 命令仅作历史记录，不再执行。新版 `acceptance-smoke.py` 支持最多 10000 文件，仅用于本地独立 Linux 虚拟机；安装及运行见 [虚拟机验收](VM-ACCEPTANCE.md)。新增限额扫描检查和扫描子进程资源记录。5000/10000 规模尚待实跑，不能当作已通过。
